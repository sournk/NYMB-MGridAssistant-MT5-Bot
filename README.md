...
- [Тех. задание](#тех-задание)
  - [V3 от 2026-01-14 - Частичные TP](#v3-от-2026-01-14---частичные-tp)
  - [V2 от 2025-04-28](#v2-от-2025-04-28)
  - [V1](#v1)
- [Стратегия](#стратегия)
...
- [Параметры бота](#параметры-бота)
      - [1. СЕТАП (SET)](#1-сетап-set)
      - [2. УВЕДОМЛЕНИЯ (NTF)](#2-уведомления-ntf)
      - [3. ГРАФИКА (GRH)](#3-графика-grh)
      - [4. ЭМУЛЯЦИЯ В ТЕСТЕРЕ (TST)](#4-эмуляция-в-тестере-tst)
      - [5. MISC (MS)](#5-misc-ms)
...
## Что нового?
```
4.00: [+] Параметры для доли объема каждого частичного закрытия SET_TRD_PTP_V_*
...
### V3 от 2026-01-14 - Частичные TP

![Partial TP](img/UM004.%20V4%20Partial%20TP.png)

- [x] **Частичное закрытие ОСНОВНОЙ и ХЭДЖ позиции по TP:**
     - [x] Новый параметр `SET_TRD_PTP` - количество частичных закрытий TP
     - [x] Бот разобьет исходный TP на `SET_TRD_PTP` частей
     - [x] При достижении каждого уровня бот закроет часть позиции, пропорционально номеру TP, который достигли. Если задали 2 части, то при достижении половины исходного TP бот закроет половину суммарного лота. Если 3 части, то при достижении первого TP закрываем 33%, второго еще 33% от исходного лота.
     - [x] Количество позиций на всех шагах сетки останется таким же как в текущей версии. 
     - [x] TP во всех позициях сетки всегда будет итоговый, но бот будет закрывать частично позиции сетки при достижении соответствующего уровня.
     - [x] Удвоение лота на след. шаге должно происходить от изначального лота, без учета частичных закрытий

!!! warning Важные особенности частичных закрытий
     1. При каждом частичном закрытии бот каждый раз округляет лот вниз до минимального возможного шага лота символа. При большом числе частичных закрытий позиция может быть закрыта полностью до достижения целевого TP, потому что на каждом шаге будет закрываться чуть большая часть.
     2. Активация частичного закрытия происходит при достижении цены Bid уровня. При этом исполнение частичного закрытия может происходить по Ask. Фактическая цена частичного выхода может отличаться от построенных уровней на величину спреда в момент закрытия.
     3. Частичные закрытия также как и операции с позициями сетки бот пытается выполнить несколько раз. 

### V2 от 2025-04-28

- [x] **1. Гибкая настройка множителя Мартингейла**
     Цель: добавить возможность менять множитель объёма сделок в зависимости от номера цикла.
     Параметры:
     - [x] Множитель Мартингейла с 1-го по 6-й цикл (включительно) — по умолчанию x2 (настраиваемое значение).
     - [x] Множитель Мартингейла с 7-го цикла и далее — по умолчанию x1.5 (также настраиваемое значение).

     Требование:
     Добавить в настройки два отдельных параметра:
     - [x] MartingaleMultiplier_1_6 (например, 2.0)
     - [x] MartingaleMultiplier_7_plus (например, 1.5, 1.7, 1.8 — любое значение по выбору пользователя)
     > Гибкий мультипликатор лота задается параметрами а) `SET_LOT_NXS` - шаг для применения нового мультипликатора; и б) `SET_LOT_NXR` - новый мультипликатор.
     
     Примечание:
     - [x] Переход на второй множитель начинается с седьмого цикла (если сделка по-прежнему не достигла TP).

!!! warning ВАЖНО 
     Мультипликатор лота применяется только, если хедж позиция сетки закрылась в убыток, иначе новые позиции открываются с прошлыми параметрами - мультипликатор 1.0.     

- [x] **3. Оптимизация обработки ошибок реквоты и проскальзывания**
     Цель: повысить устойчивость к реквотам и проскальзыванию.
     Требования:
     При возникновении реквоты:
     - [x] Бот должен автоматически повторять попытку открытия сделки каждые 2 секунды.
     - [x] Продолжать попытки до успешного открытия либо до достижения максимального количества попыток (например, 5–10).
     - [x] При возникновении проскальзывания, превышающего заданный предел:
     - [x] Бот также должен постоянно перезапускать попытку открытия сделки через 2 секунды, до достижения нормальных условий или достижения лимита попыток.

     > Повторное исполнение торговых операций при ошибках опредедяется параметрами а) `SET_TRD_REP` - пколичество повторов при ошибке и б) `SET_TRD_RET` - задержка перед следующим повтором.

     > В логах выводится каждая попыка торговой операции с указанием `RET_CODE` и описания.
          ![Repeat](img/UM003.%20RepeatTrades.png)

     Примечание:
     - [x] Если возможно оптимизировать процесс повторных попыток открытия, так как бывало что во время  работы сделка пере открывалась так как из за ошибки и остановки бота логика работы нарушалась циклы прикрывались и получался убыток
...
- [x] `SET_LOT_RAT`: Мультипликатор лота
- [x] `SET_MTP_RRD`: TP RR Основной позиции по умолчанию
- [x] `SET_HSL_RAT`: Коэф. SL для Хедж позиции от дистанции TP
- [x] `SET_TRD_PTP_M`: V3: Количество частичных закрытий по TP ОСНОВНОЙ позиции
- [x] `SET_TRD_PTP_H`: V3: Количество частичных закрытий по TP ХЭДЖ позиции
- [x] `SET_TRD_PTP_V_M`: V4: Объем закрытия частичных TP ОСНОВНОЙ позиции, %
- [x] `SET_TRD_PTP_V_H`: V4: Объем закрытия частичных TP ХЭДЖ позиции, %
