...
- [ ] **1. Гибкая настройка множителя Мартингейла**
     Цель: добавить возможность менять множитель объёма сделок в зависимости от номера цикла.
     Параметры:
     - [ ] Множитель Мартингейла с 1-го по 6-й цикл (включительно) — по умолчанию x2 (настраиваемое значение).
     - [ ] Множитель Мартингейла с 7-го цикла и далее — по умолчанию x1.5 (также настраиваемое значение).

     Требование:
     Добавить в настройки два отдельных параметра:
     - [ ] MartingaleMultiplier_1_6 (например, 2.0)
     - [ ] MartingaleMultiplier_7_plus (например, 1.5, 1.7, 1.8 — любое значение по выбору пользователя)

     Примечание:
     - [ ] Переход на второй множитель начинается с седьмого цикла (если сделка по-прежнему не достигла TP).

- [ ] **3. Оптимизация обработки ошибок реквоты и проскальзывания**
     Цель: повысить устойчивость к реквотам и проскальзыванию.
     Требования:
     При возникновении реквоты:
     - [ ] Бот должен автоматически повторять попытку открытия сделки каждые 2 секунды.
     - [ ] Продолжать попытки до успешного открытия либо до достижения максимального количества попыток (например, 5–10).
     - [ ] При возникновении проскальзывания, превышающего заданный предел:
     - [ ] Бот также должен постоянно перезапускать попытку открытия сделки через 2 секунды, до достижения нормальных условий или достижения лимита попыток.

     Примечание:
     - [ ] Если возможно оптимизировать процесс повторных попыток открытия, так как бывало что во время  работы сделка пере открывалась так как из за ошибки и остановки бота логика работы нарушалась циклы прикрывались и получался убыток
